"""Production-resolution characterisation lock for the vortex Bessel-Gauss atlas.

Stage 5.5 additive test.  This lock guards the paper-preset (production) baseline:
  - N=2048, device_downsample=1 (full SLM resolution, no coarsening)
  - axial_points=181, axial_range=360 µm, axial_target_factor=3.0
  - ideal_N=1024, ideal_dx=0.18 µm
...
def test_prod_baseline_files_exist() -> None:
    """All 8 production-preset baseline files must be present in baselines_prod/."""
    case_ids = {path.stem for path in _baseline_files()}
    assert case_ids == EXPECTED_CASES, (
        f"Missing: {EXPECTED_CASES - case_ids}  Extra: {case_ids - EXPECTED_CASES}"
    )


@pytest.mark.parametrize("baseline_path", _baseline_files(), ids=lambda path: path.stem)
def test_prod_characterisation_lock_matches_baseline(baseline_path: Path) -> None:
    """Production-preset physics must match the captured baseline bit-for-bit."""
    ...
    if config_diffs:
        ...
        pytest.fail(
            f"[prod] {case_id} config provenance changed at {key}: "
            f"expected {_short(expected)}, got {_short(actual)}"
        )

    result = bt.run_case(
        config,
        preset=baseline["case_definition"]["preset"],
        path=path,
        case_id=case_id,
    )
