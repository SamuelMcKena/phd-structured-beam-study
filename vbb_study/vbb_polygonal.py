...
def target_polygon_ring_amplitude(grid: Mapping[str, Any], config: PolygonalVortexConfig) -> np.ndarray:
    ...
    return np.exp(-0.5 * (distance / max(float(config.ring_width_m), bt.EPS)) ** 2)
...
def _support_from_polygon(config: PolygonalVortexConfig) -> tuple[np.ndarray, np.ndarray]:
    ...
    nx = dy
    ny = -dx
    norm = np.maximum(np.hypot(nx, ny), bt.EPS)
    nx = nx / norm
    ny = ny / norm
...
def angular_ring_support(grid: Mapping[str, Any], config: PolygonalVortexConfig) -> np.ndarray:
    ...
    sigma = max(float(config.k_ring_width_fraction) * float(config.kr_m_inv), bt.EPS)
...
def refined_polygonal_field(
    ...
    for iteration in range(int(max(0, config.refinement_iterations))):
        ...
        if iteration == 0 or (iteration + 1) % 10 == 0 or iteration + 1 == int(config.refinement_iterations):
            image = np.abs(bt.ifft2c(spectrum))
            image = image / (float(np.max(image)) + bt.EPS)
...
def _autocorrelation_side_peak(image: np.ndarray, grid: Mapping[str, Any], exclusion_radius_m: float) -> float:
    ...
    corr = corr / (float(np.max(corr)) + bt.EPS)
    N = corr.shape[0]
...
def polygonal_ring_metrics(
    ...
    *,
    ...
    peak = float(np.max(intensity)) + bt.EPS
    norm = intensity / peak
    ...
    localization = float(np.sum(intensity[R <= surround_radius]) / (np.sum(intensity) + bt.EPS))
    ...
    if expected < other.size:
        other[expected] = 0.0
    contour_order_fidelity = float(order_amp / (float(np.max(other)) + order_amp + bt.EPS))
    ...
    edge_uniformity = float(1.0 / (1.0 + np.std(contour_values) / (np.mean(contour_values) + bt.EPS)))

    center = norm[norm.shape[0] // 2, norm.shape[1] // 2]
    core_null_depth = float(center / (np.max(contour_values) + bt.EPS))
    winding = (
    ...
    return {
        ...
        "corner_to_edge_fluence_ratio": float(corner_mean / (edge_mean + bt.EPS)) if np.isfinite(corner_mean) and np.isfinite(edge_mean) else np.nan,
        "flat_radius_um": float(config.flat_radius_m / bt.um),
        "corner_radius_um": float(corner_radius / bt.um),
        "surround_radius_um": float(surround_radius / bt.um),
...
def hollow_hex_side_lobe_metrics(
    ...
    *,
    ...
    return {
        ...
        "ring_energy_fraction": float(np.sum(intensity[ring_guard]) / (np.sum(intensity[eval_mask]) + bt.EPS))
        if np.any(eval_mask)
        else np.nan,
        "ring_mean_fraction": float(np.mean(norm[ring_mask])) if np.any(ring_mask) else np.nan,
        "side_lobe_energy_fraction": float(np.sum(intensity[side_mask]) / (np.sum(intensity[eval_mask]) + bt.EPS))
        if np.any(eval_mask)
        else np.nan,
        "peak_radius_um": float(R[peak_idx] / bt.um),
        "target_flat_radius_um": float(config.flat_radius_m / bt.um),
        "target_corner_radius_um": float(corner_radius / bt.um),
        "core_mask_radius_um": float(core_radius / bt.um),
    }
...
def _slm_polygonal_phase(grid: Mapping[str, Any], design: bt.BeamDesign, config: PolygonalVortexConfig, phase_table: Mapping[str, np.ndarray]) -> np.ndarray:
...
def build_lab_realistic_case(
    ...
    *,
    ...
    target_core = 2.0 * 2.405 / max(float(config.kr_m_inv), bt.EPS)
    cfg = replace(
        twin_config,
        target=replace(
            twin_config.target,
    ...
    rect_grid = bt.make_rect_grid(nx, ny, cfg.slm.pixel_pitch_m * ds)
    phase = _slm_polygonal_phase(rect_grid, design, config, phase_table)
    if cfg.include_blaze:
        phase = phase + bt.TWOPI * cfg.slm.carrier_cpm * np.asarray(rect_grid["X"], dtype=float)
    phase_wrapped = bt.phase_wrap(phase)
    phase_quantized = bt.quantize_phase(phase, int(config.phase_bits))
    amp = bt.gaussian_amplitude(rect_grid["R"], cfg.laser.beam_radius_on_slm_m)
    fill = bt.fill_factor_amplitude(rect_grid, cfg.slm.pixel_pitch_m, cfg.slm.fill_factor) if cfg.include_fill_factor else np.ones_like(amp)
    aperture = (
    ...
    pupil_grid = bt.make_xy_grid(cfg.grid.N, rect_grid["dx"])
    U = _pad_rect_to_square(U_rect, int(cfg.grid.N))
    if cfg.include_first_order_isolation and cfg.include_blaze:
        filter_geometry = bt.first_order_filter_geometry(pupil_grid, cfg.slm, design)
    ...
    if cfg.apply_interface:
        W = bt.interface_aberration_pupil(pupil_grid, cfg.laser, cfg.objective, cfg.material)
        if cfg.correct_interface:
            W = W + bt.interface_correction_phase(pupil_grid, cfg.laser, cfg.objective, cfg.material)
        U = U * np.exp(1j * W)
    U_focus, focal_grid = bt.focus_to_focal_plane(U, pupil_grid, cfg.laser, cfg.objective)
    volume = bt.propagate_volume(
        U_focus,
        focal_grid,
    ...
    gray = bt.phase_to_gray(phase_quantized, int(config.phase_bits), invert=cfg.slm.invert_gray)
    return {
        "path": "lab_realistic",
        "config": config,
        "twin_config": cfg,
...
def material_hex_ring_metrics(case: Mapping[str, Any], pulse_energy_J: float, threshold_J_cm2: float) -> dict[str, Any]:
    ...
    return {
        "threshold_area_um2": float(np.count_nonzero(mask) * float(grid["dx"]) ** 2 / (bt.um**2)),
        "edge_fluence_uniformity": float(1.0 / (1.0 + np.std(contour_f) / (np.mean(contour_f) + bt.EPS))),
        "corner_to_edge_fluence_ratio": float(np.mean(contour_f[corner]) / (np.mean(contour_f[edge]) + bt.EPS)),
        "dark_core_radius_um": float(0.45 * case["config"].flat_radius_m / bt.um),
        "fluence_xy_J_cm2": F,
        "threshold_mask": mask,
    }
...
def plot_hex_ring_checkpoint(ideal: Mapping[str, Any], lab: Mapping[str, Any], output_path: str | Path) -> Path:
    ...
    for col, (case, label) in enumerate(zip(cases, labels)):
        ...
        axes[0, col].plot(contour["x_m"] / bt.um, contour["y_m"] / bt.um, color="white", lw=0.9, alpha=0.9)
        axes[0, col].set_title(label)
        axes[0, col].set_xlabel("x [um, sample plane]")
        axes[0, col].set_ylabel("y [um, sample plane]")
        vol = case["volume"]
        gx = np.asarray(vol["crop_grid"]["x"], dtype=float) / bt.um
