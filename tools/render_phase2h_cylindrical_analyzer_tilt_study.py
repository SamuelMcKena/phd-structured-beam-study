...
def _figure_atlas(mode: str, ell: int, cases: dict[float, tuple[VectorField, dict[int, np.ndarray]]], outdir: Path) -> None:
    ...
    for row, tilt in enumerate(TILTS_DEG):
        ...
        for col, angle in enumerate(ANALYZERS_DEG):
            axes[row, col].imshow(frames[angle] / max(global_peak, np.finfo(float).tiny), origin="lower", extent=extent, vmin=0.0, vmax=1.0, aspect="equal")
            axes[row, col].plot(m.centroid_x_m * 1e3, m.centroid_y_m * 1e3, "+", markersize=7)
    ...
    fig.suptitle(
        ...
        f"x-axis rigid axicon tilt; zref={Z_REF_MM:.0f} mm; common intensity scale; SIMULATION ONLY",
        fontsize=13,
    )
    fig.savefig(outdir / f"analyzer_atlas_{mode}_ell{ell}_x_tilt.png", dpi=180)
    plt.close(fig)
...
def _figure_summary(rows: list[dict], outdir: Path) -> None:
    ...
    for mode in MODES:
        for ell in ELLS:
            for direction in ("x", "y"):
                subset = [r for r in rows if r["mode"] == mode and r["ell"] == ell and r["direction"] == direction and r["analyzer_deg"] == 0]
                subset = sorted(subset, key=lambda r: float(r["tilt_deg"]))
                label = f"{mode}, ell={ell}, {direction}-tilt"
                axes[0, 0].plot([r["tilt_deg"] for r in subset], [r["petal_count"] for r in subset], marker="o", label=label)
                axes[0, 1].plot([r["tilt_deg"] for r in subset], [r["modulation_fraction"] for r in subset], marker="o", label=label)
                axes[1, 0].plot([r["tilt_deg"] for r in subset], [r["petal_orientation_deg"] for r in subset], marker="o", label=label)
    ...
    axes[0, 1].set_ylabel("petal modulation fraction")
    axes[1, 0].set_ylabel("petal orientation (deg)")
    axes[1, 1].set_ylabel("analysis-ring radius (µm)")
    for ax in axes.flat:
        ax.set_xlabel("rigid axicon tilt (deg)")
        ax.grid(alpha=0.25)
    axes[0, 0].legend(fontsize=7, ncol=2)
    fig.suptitle("Phase 2H cylindrical-vector analyzer response to axicon tilt\n0° analyzer shown in summary; full 0/45/90/135 data are in CSV", fontsize=13)
...
def main() -> None:
    ...
    expected_rows = [r for r in rows if abs(float(r["tilt_deg"])) <= 0.5]
    retention = float(np.mean([bool(r["expected_harmonic_retained"]) for r in expected_rows]))
    payload = {
    ...
    print(json.dumps({"output_dir": str(outdir), "row_count": len(rows), "small_tilt_retention_fraction": retention}, indent=2))
