...
def _build_case(bundle: CalibrationBundle, config: NathanHexagonConfig, direction: str, tilt_deg: float) -> dict:
...
def _metric_row(direction: str, tilt_deg: float, route: dict, long: dict, nominal_peak: float, nominal_power: float) -> dict[str, float | str | bool]:
    ...
    sx = float(np.polyfit(z_m, cx_m, 1)[0])
    sy = float(np.polyfit(z_m, cy_m, 1)[0])
    steering = math.atan(math.hypot(sx, sy))
    ...
    return {
        ...
        "peak_over_nominal_0deg": m.peak_intensity / max(nominal_peak, np.finfo(float).tiny),
        "power_over_nominal_0deg": m.power_au_m2 / max(nominal_power, np.finfo(float).tiny),
        "centroid_slope_x_m_per_m": sx,
        "centroid_slope_y_m_per_m": sy,
        "steering_angle_mrad": steering * 1e3,
...
def _figure_response(rows: list[dict], outdir: Path) -> None:
    ...
    fields = (
        ("centroid_x_mm", "centroid x at 30 mm (mm)"),
        ("centroid_y_mm", "centroid y at 30 mm (mm)"),
        ("steering_angle_mrad", "steering magnitude (mrad)"),
    ...
    for ax, (key, ylabel) in zip(axes.flat, fields):
        for direction in ("x", "y"):
            ...
            ax.plot([r["tilt_deg"] for r in subset], [r[key] for r in subset], marker="o", label=f"{direction}-tilt")
        ax.axvline(0.0, linewidth=0.8)
        ax.set_xlabel("rigid axicon tilt (deg)")
        ax.set_ylabel(ylabel)
        ax.grid(alpha=0.25)
    axes[0, 0].legend()
    fig.suptitle("Phase 2H systematic six-sector vector tilt response\nsynthetic 2°-base-angle axicon; primary alignment sweep only; zref=30 mm", fontsize=13)
...
def _figure_longitudinal(direction: str, representative: dict[float, dict], outdir: Path) -> None:
    ...
    fig.suptitle(f"Phase 2H six-sector {direction}-tilt longitudinal evidence\ncommon scale; dashed line is canonical zref={Z_REF_MM:.0f} mm", fontsize=13)
...
def _figure_zref_profiles(direction: str, representative: dict[float, dict], outdir: Path) -> None:
    ...
    fig.suptitle(f"Phase 2H six-sector {direction}-tilt canonical profile evidence\nall curves use the same 0° peak normalisation", fontsize=13)
...
def main() -> None:
    ...
    # Build the zero-tilt reference once; its result is identical for x/y labels.
    zero_route = _build_case(bundle, config, "x", 0.0)
    ...
    for direction in ("x", "y"):
        for tilt in ALL_TILTS_DEG:
            if tilt == 0.0:
                ...
                route = _build_case(bundle, config, direction, tilt)
                long = _longitudinal(route["post_axicon"])
            rows.append(_metric_row(direction, tilt, route, long, zero_m.peak_intensity, zero_m.power_au_m2))
    ...
    print(json.dumps({"output_dir": str(outdir), "case_count": len(rows), "canonical_z_ref_mm": Z_REF_MM}, indent=2))
